...
class TestProcessFile:
    ...
    @pytest.mark.skip(reason="Requires complex API mocking - ImageConfig not available in current SDK version")
    def test_image_generation_with_aspect_ratio(self):
        """Test image generation with aspect ratio config.

        Note: This test is skipped because it requires mocking internal SDK
        types (ImageConfig, GenerateContentConfig) that may not exist in all
        SDK versions. The functionality is tested via integration tests.
        """
        pass
