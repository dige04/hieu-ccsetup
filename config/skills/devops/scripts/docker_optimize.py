...
class DockerfileAnalyzer:
    ...
    def analyze_base_image(self) -> None:
        ...
        for i, line in enumerate(self.lines, 1):
            ...
            if line.startswith('FROM'):
                # Extract image part (e.g., "node:20-alpine" from "FROM node:20-alpine AS builder")
                parts = line.split()
                image_part = parts[1] if len(parts) >= 2 else ''

                # Check for 'latest' tag or no tag at all
                # Valid tags: :20-alpine, :3.9-slim, @sha256:...
                has_specific_tag = (':' in image_part and not image_part.endswith(':latest')) or '@' in image_part
    ...
    def analyze_combine_run(self) -> None:
        ...
        # Handle consecutive RUN commands at end of file
        if consecutive_runs > 1:
            self.suggestions.append({
                'line': first_run_line,
                'category': 'layers',
